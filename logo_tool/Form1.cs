    ...
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            foreach(RadioButton rd in gr_size.Controls)
            {
                ...
                if (rd.Checked)
                {
                    if (rd.Name.IndexOf("600") > 0)
                    {
                        size = 600;
                    }
                    else if (rd.Name.IndexOf("800") > 0)
                    {
                        size = 800;
                    }
            ...
            if(size != 0)
            {
                string logo_resize = resize(lbl_logo.Text.Trim(), size, cb_style_resize.Checked);
                lbl_logo.Text = logo_resize;
            }

            string logo_result_path = render_logo(lbl_result_folder.Text, lbl_logo.Text, lbl_yoda.Text, lbl_bottle.Text, lbl_hand.Text);
        ...
        private string render_logo(string folder_result,string logo_p, string yoda_p, string bottle_p, string hand_p)
        {
            ...
            using (MagickImage backgroud = new MagickImage(logo_p))
            {
                ...
                backgroud.Composite(yoda, 0, 500, CompositeOperator.Over);
                backgroud.Composite(bottle, 200, 700, CompositeOperator.Over);
                backgroud.Composite(hand, 100, 750, CompositeOperator.Over);
